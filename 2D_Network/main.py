...
def main():
    ...
    args.lambda_av = args.lambda_av * 3
    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            train_loss_kl = checkpoint['train_loss_kl']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        tr_acc1, tr_acc5, tr_loss_kl, tr_loss = train(train_loader, model, criterion, optimizer, epoch, log_training, scaler)
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            ...
            if not args.distributed or (args.distributed and torch.distributed.get_rank() == 0):
                ...
                train_loss_kl.append(tr_loss_kl)
                ...
                df = pd.DataFrame({'val_acc_top1': val_acc_top1, 'val_acc_top5': val_acc_top5, 
                                    ...
                                    'train_loss_kl': train_loss_kl, 'train_loss': train_loss, 
...
def train(train_loader, model, criterion, optimizer, epoch, log, scaler=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if r < args.MCA_prob:
            ...
            lam = torch.from_numpy(np.random.beta(args.beta, args.beta, batch_num)).view(-1,1,1,1,1)
            if args.amp:
                lam = lam.half()
            ...
            input_color = input_var.view((-1, args.num_segments, 3) + input_var.size()[-2:])
            input_color = lam * input_color + (1-lam) * input_color[:, :, rand_index]
            input_color = input_color.view((-1, 3*args.num_segments) + input_color.size()[-2:])
            # construct training pair for Variation Alignment
            input_cat = torch.cat((input_var,input_color),0)
        ...
        if args.amp:
            with autocast():
                ...
                if r < args.MCA_prob:
                    ...
                    loss = criterion(output[:batch_num], target_var)
                    loss_kl = args.lambda_av * nn.KLDivLoss(reduction='batchmean')(nn.LogSoftmax(dim=1)(output[batch_num:]), nn.Softmax(dim=1)(output[:batch_num].detach()))
                    loss = loss + loss_kl
                else:
        ...
        if r < args.MCA_prob:
            prec1, prec5 = accuracy(output[:batch_num].data, target, topk=(1, 5))
            losses_kl.update(loss_kl.item(), input.size(0))
        else:
        ...
        if not args.distributed or (args.distributed and torch.distributed.get_rank() == 0):
            if i % args.print_freq == 0:
                output = ('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                    ...
                    data_time=data_time, loss=losses, loss_kl=losses_kl, top1=top1, top5=top5, lr=optimizer.param_groups[-1]['lr'] * 0.1))  # TODO
                print(output)
                log.write(output + '\n')
                log.flush()

    return top1.avg, top5.avg, losses_kl.avg, losses.avg
