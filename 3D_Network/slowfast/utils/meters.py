...
class TrainMeter_KL(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.loss = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_total = 0.0
        self.loss_kl = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_kl_total = 0.0
        self.lr = None
        self.grad_norm = None
        # Current minibatch errors (smoothed over a window).
        self.mb_top1_err = ScalarMeter(cfg.LOG_PERIOD)
    ...
    def update_stats(
        self, top1_err, top5_err, loss_kl, loss, lr, grad_norm, mb_size, multi_loss=None
    ):
        """
        Update the current stats.
        Args:
        ...
        """
        self.loss.add_value(loss)
        self.loss_kl.add_value(loss_kl)
        self.lr = lr
        self.grad_norm = grad_norm
        self.loss_total += loss * mb_size
        self.loss_kl_total += loss_kl * mb_size
        self.num_samples += mb_size
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "eta": eta,
            "loss": self.loss.get_win_median(),
            "loss_kl": self.loss_kl.get_win_median(),
            "lr": self.lr,
            "grad_norm": self.grad_norm,
            "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
        }
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            ...
            top5_err = self.num_top5_mis / self.num_samples
            avg_loss = self.loss_total / self.num_samples
            avg_loss_kl = self.loss_kl_total / self.num_samples
            stats["top1_err"] = top1_err
            stats["top5_err"] = top5_err
            stats["loss"] = avg_loss
            stats["loss_kl"] = avg_loss_kl
        if self.multi_loss is not None:
            avg_loss_list = self.multi_loss.get_global_avg()
            for idx, loss in enumerate(avg_loss_list):
                stats["loss_" + str(idx)] = loss
