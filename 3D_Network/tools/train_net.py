...
from slowfast.utils.meters import AVAMeter, EpochTimer, TrainMeter_KL, ValMeter
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(
        ...
        ######  MCA  ######
        r = np.random.rand(1)
        if r < cfg.TRAIN.MCA_PROB:
            # generate co-efficient lambda
            batch_num = inputs[0].shape[0]
            lam = torch.from_numpy(np.random.beta(cfg.TRAIN.BETA, cfg.TRAIN.BETA, batch_num)).view(-1,1,1,1,1).half().cuda()
            ...
            inputs_color_0 = lam * inputs[0] + (1-lam) * inputs[0][:, rand_index]
            inputs_color_1 = lam * inputs[1] + (1-lam) * inputs[1][:, rand_index]
            # construct training pair for Variation Alignment
            inputs_cat = []
            inputs_cat.append(torch.cat((inputs[0],inputs_color_0),0))
            inputs_cat.append(torch.cat((inputs[1],inputs_color_1),0))
            inputs = inputs_cat
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if r < cfg.TRAIN.MCA_PROB:
                loss = loss_fun(preds[:batch_num], labels)
                loss_kl = cfg.TRAIN.LAMBDA_AV * nn.KLDivLoss(reduction='batchmean')(nn.LogSoftmax(dim=1)(preds[batch_num:]), nn.Softmax(dim=1)(preds[:batch_num].detach()))
                loss = loss + loss_kl
            else:
        ...
        if cfg.MIXUP.ENABLE:
            ...
            preds = preds.detach()
            if r < cfg.TRAIN.MCA_PROB:
                preds[:batch_num][idx_top1] += preds[:batch_num][idx_top2]
                preds[:batch_num][idx_top2] = 0.0
            else:
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                if r < cfg.TRAIN.MCA_PROB:
                    # Compute the errors.
                    num_topks_correct = metrics.topks_correct(preds[:batch_num], labels, (1, 5))
                    top1_err, top5_err = [
                        (1.0 - x / preds[:batch_num].size(0)) * 100.0 for x in num_topks_correct
                    ]
                    # Gather all the predictions across all the devices.
                    if cfg.NUM_GPUS > 1:
                        loss_kl, loss, grad_norm, top1_err, top5_err = du.all_reduce(
                            [loss_kl.detach(), loss.detach(), grad_norm, top1_err, top5_err]
                        )

                    # Copy the stats from GPU to CPU (sync point).
                    loss_kl, loss, grad_norm, top1_err, top5_err = (
                        loss_kl.item(),
                        loss.item(),
                        grad_norm.item(),
                        top1_err.item(),
...
def build_trainer(cfg):
    ...
    # Create meters.
    train_meter = TrainMeter_KL(len(train_loader), cfg)
